...
async function fetchTSDB(key) {
  const k = key || '3';
  const url = `https://www.thesportsdb.com/api/v1/json/${k}/eventsseason.php?id=${TSDB_LEAGUE}&s=${TSDB_SEASON}`;
...
async function syncGroups(key) {
  const events = await fetchTSDB(key);
  return Object.assign({ totalEventos: events.length }, mapGroupResults(events));
}

module.exports = { syncGroups, fetchTSDB, mapGroupResults, toOurName, GROUP_PAIR_INDEX };
